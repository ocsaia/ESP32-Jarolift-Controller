...
float Dusk2Dawn::hourAngleSunrise(float lat, float solarDec, float zenith) {
  ...
  // acos() of anything outside [-1, 1] is NaN, which is how "the sun never
  // reaches this angle today" arrives here - polar day and night, and, for the
  // twilight angles, ordinary summer nights at mid-northern latitudes.
  // sunriseSet() turns that into its no-event result.
  float HA     = acos(HAarg);
